...
def diligencias(request):
    ...
    diligencias_sentencas_comuns = Diligencia.objects.filter(prioridade=False).filter(tipo='Sentença')
    diligencias_sentencas_prioridade = Diligencia.objects.filter(prioridade=True).filter(tipo='Sentença')
